...
**v1.3.x territory:**
18. **X4** — Bundle a real web chat UI (static SPA served from Runner's Kestrel, reusing existing `/api/chat` endpoints)
19. **Runner tab restructure** — follow-up to X2 once F3's tabbed aesthetic lands
20. **X5** — GPU/CPU compute indicator (read-only first, selector later)
...
- **X15** — revisit RAG file-size and chunk-size caps so large DCS airframe manuals (Chuck's Guides, up to ~180 MB and ~900 pages) ingest cleanly. Investigation + tuning pass — paired follow-up to X14. Not near-term; slot after F3 once the v1.2.x patch stream clears. *(rescoped 2026-04-19 RAG audit into 4 stages — see X15 entry)*
...
Items `B1`â€“`F4` below were triaged from Stephen's `Downloads/# Free-AI-SSD Project TODO.md` (dictated-while-driving notes — treat TODO assumptions with skepticism). Items `X1`â€“`X5` were added 2026-04-18 from the v1.2.1 field-test findings (`C:\Users\Kninetimmy\Documents\ai ssd issues.txt`). Each section is addressable independently.
...
### F2a — Model picker UX gaps surfaced on v1.3.5 field test
...
- `shared/ViewModels/PrepViewModel.cs` — rename `PullInstallCommand` â†’ `DownloadCommand`, semantics .Take(1) â†’ all checked; delete `PullSelectedCommand`, `VerifyCommand`, `AddStarterModelsCommand`; auto-verify in `PullModelsAsync`; add `ModelRow.Status`
...
- **Stage 1 âœ…** — VM updates (rename + delete + auto-verify). Commit `26d9a14`.
- **Stage 2 âœ…** — XAML rewrite to 2 tabs with merged grid + verbage.
- **Stage 3 âœ…** — FTUE re-target + Runner tooltip + doc updates.
...
- `runner/Services/PttVoicePipelineService.cs:256â€“272` — replace polling loop with `await` on the new contract
...
**âš  Watch for:** the fallback `SystemTextToSpeechService` uses `System.Speech.Synthesis` which has its own `SpeakCompleted` event — wire that similarly. Don't leave one of the two TTS implementations on the old polling contract.
...
**Symptom:** Both Start and Stop Ollama buttons are always visible; the Stop button uses `TactileMagentaButton` style unconditionally (`runner/MainWindow.xaml:236â€“250`), so it looks like the active CTA even when Ollama is already running. Confusing — users think the magenta "Stop" button means "Ollama isn't running, click here."
...
**Affected files:**
- `runner/MainWindow.xaml:236â€“250` — button style/visibility bindings
- Possibly `runner/MainWindow.xaml.cs:374â€“418` — Start_Click / Stop_Click may need to trigger a PropertyChanged for `IsRunning` if the binding doesn't pick it up automatically
- `tests/` — N/A (visual)
...
**Root cause (verified 2026-04-18):**
- `runner-core/Services/ChatService.cs` — catch returns `new ChatResponse(string.Empty, null, false)` on any exception. Streaming path is slightly better (injects `[Error: â€¦]` into the token stream when partial content exists) but still returns success-shaped object.
...
**Affected files:**
- `runner-core/Services/IChatService.cs`, `ChatService.cs` — new result type; update all callers.
- `runner-core/Services/ISpeechToTextService.cs`, `runner/Services/WhisperSpeechToTextService.cs` — same.
- `runner-core/Services/RunnerLocalApiService.cs` — error response mapping.
...
**Expansion 2026-04-19 (RAG audit fallout):**
- Add a `ChatResult.RagRetrievalFailed(innerError)` variant distinct from "no hits above threshold." `ChatService.cs:159-165` currently catches the retrieval exception, logs `"RAG retrieval skipped: {ex.Message}"`, and returns an ungrounded answer with `usedContext=false` — indistinguishable from the threshold case. UI renders a visible warning banner ("Answering without document context — retrieval failed: â€¦") on the failure variant. LAN API response carries a header or JSON field (e.g. `X-RAG-Status: retrieval-failed` vs `retrieval-empty`) so API consumers can react differently.
...
**Status:** Done — PR #157 (`449ec2e`), merged 2026-04-19. Stages 1â€“2 shipped; Stage 3 is X21b.
...
**Scope:** PrepApp detects mismatch on model config change â†’ warning dialog "Library was indexed with model X; switched to Y — reindex required" â†’ one-click reindex using X10's rebuild-from-stored path.
