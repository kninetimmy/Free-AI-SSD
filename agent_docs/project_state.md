...
Last updated: 2026-05-06 (PR #181 follow-up branch)
...
PR #181 follow-up hardening is being prepared on a separate branch: Mac Network
Mode runtime config forces `networkModeEnabled = true` for the sidecar
handshake without persisting it; `mac-runner-host` fails startup instead of
emitting `ready: ` when the API did not actually start; `RunnerLocalApiService`
static files resolve from the published app/content root instead of
`<ssdRoot>/wwwroot`; and Mac host binary resolution chmods/rechecks executable
candidates before falling through. `dotnet` was unavailable locally, so CI
must verify the .NET build/tests before merge.
...
1. Verify/land the PR #181 follow-up hardening branch with CI.
...
6. **X4** is unblocked from the host side: MAC6 wired the static-file middleware on both platforms (PR #181); X4 only needs to drop SPA assets at `runner-core/wwwroot/chat/`.
7. For non-Mac work, pick from `H3`, `F4` follow-up, `B2`, `F2`, or `R1 Stage 2`.
...
2026-05-06 (MAC6 Mac LAN API host - PR #181, `b9815e4`) - Drafted `agent_docs/mac6_execution_prompt.md` locking in the sidecar architectural decision (net8.0 host process spawned by Swift mac-runner, reusing `RunnerLocalApiService` byte-for-byte; encrypted-config IO stays Swift-authoritative; unlock material flows over stdin only, plaintext never touches disk on Mac; X4 plumbing only, not the SPA itself). Delegated implementation to a general-purpose agent in the background; agent shipped `mac-runner-host/`, `runner-core/Services/NoOp{SpeechToText,Tts}*.cs`, static-files middleware in `RunnerLocalApiService`, `mac-runner/Sources/MacRunnerHostController.swift`, Network Mode toggle in `main.swift`, full CI wiring, and tests. Agent integrated Gemini review feedback in commit `b9815e4`; CI was green and user merged PR #181. Deferred manual-smoke gaps: Windows Companion -> real-Mac Runner over LAN with Bearer auth; Network Mode toggle with real Ollama serving a real model.
...
- **PR #181 follow-up verification:** run .NET build/tests in CI because local `dotnet` was unavailable.
