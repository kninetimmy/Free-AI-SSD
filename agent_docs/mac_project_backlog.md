...
### C7 - Encrypted-drive Manage Models unlock UX (Mac + Windows)

**Status:** **done** — C7 PR (2026-05-12). Mac PrepApp + Windows PrepApp shipped together; Windows HF-token-writeback deferred to W5.

**What shipped:**

**Mac PrepApp (full):** `ManageModelsStepView.swift` banner now hosts an explicit `[ Unlock… ]` button (locked state, yellow) that opens a new `UnlockSheet` mirroring Runner's `UnlockSheet` pattern (SecureField, inline error, Cancel/Unlock with default-action keyboard). On a successful unlock the banner flips green (`✓ Unlocked for this session`) and the Add disclosure + Remove buttons enable. `PrepViewModel.swift` gains `isManageModelsUnlocked` (@Published), `unlockMaterial` (private), and `unlockedConfig` (private decrypted dict cache so HF-token re-encrypts preserve unknown fields like `networkApiBindAddress` and `embeddingModel`). `attemptUnlock(password:)` is a synchronous mirror of Runner's; on success it lifts `huggingFaceToken` from the decrypted config and runs `tryMigratePlaintext` so a stale plaintext file from a pre-encryption session is absorbed or removed. HF-token persistence fires on natural boundaries only — `pullPendingTags` tail after a successful HF pull, and `exitManageModels` (Done click) — never per-keystroke. Lifecycle: zeroize on Done, on `selectedCandidate` change (before the C6 detector re-runs so a green banner can never render against a different drive's identity), and on app termination via `UnlockMaterial.deinit` (no explicit AppDelegate hook needed — `@StateObject` drop triggers it). M18's inline "Unlock required (C7) — disabled on encrypted drives." caption is removed; the banner above carries the entire state explanation.

**Windows PrepApp (UX + gate flip + HF token lift, write-back deferred):** `MainWindow.xaml` configured-drive banner gains an `[ Unlock… ]` button (locked, yellow) and a green confirmation `Border` ("Unlocked for this session. Add and Remove enabled.") under it. Both bind to new VM properties `ShowUnlockButton` and `ShowSessionUnlockedBanner`. New `UnlockDialog.xaml`/`.cs` mirrors Runner's `UnlockDriveDialog` with the addition of an inline error TextBlock so the caller can re-show the dialog with "Incorrect password" feedback without a MessageBox interrupt. `MainWindow.xaml.cs:OnUnlockRequested` handles the dialog loop and calls `SsdEncryption.TryUnlockPortableConfigWithMaterial` + `TryMigratePlaintextAsync`, then `vm.ApplyUnlockResult(config, material)`. `PrepViewModel.cs` gains `IsManageSessionUnlocked` + `UnlockRequested` event + `UnlockCommand` + `ApplyUnlockResult` + `LockManageSession`. `CanMutateDrive` updated to `!_isModelOperationRunning && (!_isSelectedDriveEncrypted || _isManageSessionUnlocked)`. `OnSelectedDriveChanged` calls `LockManageSession()` first so drive switches zeroize before the new drive's encryption-state detection runs. `MainWindow.OnClosed` calls `LockManageSession` so app close zeroes the derived-key buffer via `CryptographicOperations.ZeroMemory`. HF token is lifted from the decrypted config on unlock (`HuggingFaceTokenInput = config.HuggingFaceToken`); **write-back path deferred to W5** because the existing Windows save path (`IModelService.SaveConfigAsync` + `PortableConfig.SaveAsync`) does not route through `IConfigStore.SaveAsync` and would require an architectural refactor out of scope for C7.

**No-touch / reused as-is:**
- `shared/SsdEncryption.cs` — `TryUnlockPortableConfigWithMaterial`, `SaveEncryptedConfigAsync`, `TryMigratePlaintextAsync` (X9 plumbing).
- `mac-runner/Sources/SsdEncryption.swift` — `tryUnlockPortableConfig`, `saveEncryptedConfig`, `tryMigratePlaintext`, `UnlockMaterial` class.
- `mac-prep-host` sidecar — zero change. `pull-model` / `remove-model` don't touch `portable-config.*` (manifests are written by Ollama directly), and `set-hf-token` is in-memory-sidecar-only.

**Tests:** 4 new Swift pins in `mac-prep-app/Tests/PrepAppTests.swift` covering the C7-specific round-trip (unlock → mutate dict → save with cached material → re-open → assert HF token persisted; round-trip preserves unknown fields; wrong-passphrase returns `.incorrectPassword` without producing material; `UnlockMaterial.zeroize` wipes derivedKey buffer). 7 new C# pins in `tests/PrepViewModelTests.cs` covering encrypted-drive-at-init invariants, `ApplyUnlockResult` flow including HF token lift, empty-token leave-untouched semantics, `LockManageSession` zeroize + gate flip, drive-change locks the session, UnlockCommand gating on unencrypted drives, and the `UnlockRequested` event raise.

**Decisions pinned:** three new entries in `project_decisions.md` (2026-05-12) covering explicit Unlock button (not auto-prompt), lock-on-Done (not lock-on-background, mirroring Runner's MAC36a removal of `willResignActiveNotification`), and commit-on-boundary HF-token persistence (not per-keystroke — exFAT over USB makes per-character AES-GCM + two-file commit user-visible as input lag).

**W5 follow-up filed (Windows HF-token write-back):** the architectural refactor to thread `IConfigStore` through `prep-app/Services/ModelService.cs` and `shared/PortableConfig.cs`'s save path so encrypted-drive HF-token edits round-trip back to the sealed blob. Mac PrepApp already does this directly via `SsdEncryption.saveEncryptedConfig`; Windows currently lifts the token on unlock but doesn't write back.

**Exit criteria (Mac priority):** plug a previously-prepped + encrypted SSD → Manage models → yellow banner with `[ Unlock… ]` button. Click Unlock → SecureField sheet. Wrong passphrase → inline red "Incorrect password." Sheet stays open. Correct passphrase → sheet dismisses, banner flips green, Add disclosure expandable with full picker, Remove buttons enabled. HF token field auto-populates from the decrypted config. Pull an HF model → succeeds, token re-persists. Click Done → returns to drive selection; re-enter same encrypted drive → banner yellow again (re-unlock required). Switch to a different drive mid-unlocked → banner flips back to yellow. **Windows priority:** same banner button + dialog flow; Add/Download/Remove commands become enabled after unlock; HF token field pre-populates on unlock; token write-back deferred (note in banner during W5 implementation if needed, or in the W5 PR body).
