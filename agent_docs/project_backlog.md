...
| C27 Stage 2-4 | (new) | Hugging Face: pull integration (Stage 2) + token auth (Stage 3) + per-quant row expansion (Stage 4) | this file |
...
**P1.5 — Picker filter cluster follow-ons (filed 2026-05-10 post-v1.3.22 Mac field test; C24/C25/C26 + C27 Stage 1 cleared, C27 Stages 2-4 remain):**
...
5. **C27 Stage 2** — HF pull integration via `hf.co/<repo>:<quant>` through Ollama's native HF pull syntax; wire `ModelManagementService.PullModelAsync` for HF tags; `siblings[].size` disk-budget warnings; cancel/resume parity with existing Ollama pulls.
